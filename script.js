...
// Store proxies in memory to avoid fetching on every request
let proxyData = null;
...
// Fetch proxies once when the worker starts
fetchAndProcessProxies();
...
export default {
    async fetch(request) {
        ...
        if (path.startsWith('/proxies/')) {
            ...
            const countryProxies = proxies[country] || [];
            return new Response(JSON.stringify(countryProxies), {
...
<head>
    ...
    <style>
      ...
      #app { background-color: #fff; padding: 2rem; border-radius: 8px; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1); width: 90%; max-width: 600px; }
      ...
      #proxy-list { height: 150px; overflow-y: auto; border: 1px solid #ccc; padding: 0.5rem; border-radius: 4px; }
      button { background-color: #ff4500; color: #fff; padding: 0.75rem 1.5rem; border: none; border-radius: 4px; cursor: pointer; font-size: 1rem; width: 100%; margin-top: 1rem; }
      button:hover { background-color: #e03e00; }
      #output { margin-top: 1.5rem; background-color: #f9f9f9; padding: 1rem; border-radius: 4px; white-space: pre-wrap; word-wrap: break-word; }
    </style>
</head>
<body>
    <div id="app">
        <h1>Red Bunny</h1>
        <form>
            <div class="form-group"><label for="host">Host:</label><input type="text" id="host"></div>
            <div class="form-group"><label for="sni">SNI:</label><input type="text" id="sni"></div>
            <div class="form-group"><label for="country">Negara:</label><select id="country"></select></div>
            <div class="form-group"><label>Daftar Proksi:</label><div id="proxy-list"></div></div>
    ...
    <script>
      document.addEventListener('DOMContentLoaded', () => {
        ...
        async function loadProxiesForCountry(country) {
          proxyList.innerHTML = 'Memuat...';
          selectedProxy = null;
          const response = await fetch('/proxies/' + country);
          ...
          proxies.forEach((proxy, index) => {
            ...
            radio.addEventListener('change', () => { selectedProxy = proxy; });
        ...
        countrySelect.addEventListener('change', (e) => loadProxiesForCountry(e.target.value));
